    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        <div className="flex gap-2 mb-2">
          ...
          />
          <Button type="button" variant="ghost" onClick={() => addSubject(subjectInput)}>
            Add
          </Button>
        </div>
...
export default function TeachersPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setModalState({ open: false, teacher: null });
    },
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Button onClick={() => setModalState({ open: true, teacher: null })}>
            + Add Teacher
          </Button>
        }
      />
        ...
        {teachers.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                {['Name', 'Phone', 'Classes', 'Subjects', 'Blood Group', 'Class Teacher', ''].map(
                  (h) => (
                    <th
                      key={h}
            ...
            <tbody className="divide-y divide-slate-100">
              {teachers.map((t) => (
                <tr key={t._id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-4 py-3">
                    <Badge color="purple">{t.bloodGroup || '—'}</Badge>
                  </td>
                  <td className="px-4 py-3 text-slate-500 text-xs">
                    {t.classTeacherOf?.class
                      ? `${t.classTeacherOf.class}-${t.classTeacherOf.section}`
                      : '—'}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={() => setModalState({ open: true, teacher: t })}
                      >
                        Edit
                      </button>
                      <button
      ...
      >
        <TeacherForm
          key={modalState.teacher?._id || 'new'}
