  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <label htmlFor="isCR" className="text-sm text-slate-700">
...
export default function StudentsPage() {
  ...
  const { data: students = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get('/students', {
          params: { class: filters.class || undefined, section: filters.section || undefined },
        })
        .then((r) => r.data),
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setModalState({ open: false, student: null });
    },
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <Button onClick={() => setModalState({ open: true, student: null })}>
            + Add Student
          </Button>
        }
      />
      ...
      <Card className="p-4 mb-4">
        <div className="flex items-center gap-4">
          ...
          {(filters.class || filters.section) && (
            <button
              onClick={() => setFilters({ class: '', section: '' })}
            ...
            <tbody className="divide-y divide-slate-100">
              {students.map((s) => (
                <tr key={s._id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <Badge color="purple">{s.bloodGroup || '—'}</Badge>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={() => setModalState({ open: true, student: s })}
                      >
                        Edit
                      </button>
                      <button
      ...
      >
        <StudentForm
          key={modalState.student?._id || 'new'}
